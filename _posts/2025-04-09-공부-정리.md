...
**요약:** 객체지향언어는 코드의 재사용성을 높이고 유지보수를 용이하게 하기 위해 설계된 프로그래밍 언어로, 핵심 개념인 캡슐화, 상속, 추상화, 다형성을 포함한다. 클래스는 객체를 정의하는 설계도로, 객체는 클래스의 인스턴스로 생성된 실제 사물이나 개념이다. 객체는 속성과 기능으로 구성되며, 메서드는 코드의 중복을 줄이고 관리하기 쉽게 해준다. 메서드는 기본형과 참조형 매개변수를 사용할 수 있으며, 인스턴스 메서드와 static 메서드로 구분된다. 인스턴스 메서드는 객체 생성 후 호출되며, static 메서드는 객체 생성 없이 클래스...
