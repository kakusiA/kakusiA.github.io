...
**요약:** 객체지향 언어는 코드의 재사용성과 유지보수의 용이성을 높이기 위해 설계된 프로그래밍 언어로, 핵심 개념으로는 캡슐화, 상속, 추상화, 다형성이 있다. 클래스는 객체를 정의하는 설계도로, 객체는 실제 존재하는 사물이나 개념을 의미한다. 객체는 클래스의 인스턴스이며, 객체의 속성과 기능을 통해 다양한 작업을 수행할 수 있다.

클래스 내에는 인스턴스 변수(IV), 클래스 변수(CV), 지역 변수(LV)가 있으며, 메서드는 코드의 중복을 줄이고 관리하기 쉽게 해준다. 메서드는 선언부와 구현부로 구성되며, 호출 시 매개변수를 통해 값을...
