...
**요약:** 객체지향 언어는 코드의 재사용성과 유지보수 용이성을 높이기 위해 설계된 프로그래밍 언어로, 객체 지향 프로그래밍(OOP)의 핵심 개념인 캡슐화, 상속, 추상화, 다형성을 포함합니다. 클래스는 객체를 정의하는 설계도로, 객체는 실제로 존재하는 사물이나 개념으로, 클래스에서 생성된 특정 객체를 인스턴스라고 부릅니다.

클래스는 데이터와 함수를 결합한 사용자 정의 타입으로, 인스턴스 변수(IV), 클래스 변수(CV), 지역 변수(LV)로 구성됩니다. 메서드는 코드의 중복을 줄이고 관리하기 쉽게 하며, 특정 기능을 수행하는 문장들의...
