...
**요약:** 객체지향언어는 코드의 재사용성과 유지보수 용이성을 높이기 위해 설계된 프로그래밍 언어로, 핵심 개념은 캡슐화, 상속, 추상화, 다형성이다. 클래스는 객체를 정의하는 설계도로, 객체는 실제 존재하는 사물이나 개념을 나타낸다. 객체는 속성과 기능으로 구성되며, 클래스에서 생성된 특정 객체는 인스턴스라고 한다.

클래스는 설계도 역할을 하며, 객체 생성 시 중복을 피할 수 있다. 변수는 인스턴스 변수(IV), 클래스 변수(CV), 지역 변수(LV)로 나뉘며, 메서드는 코드의 중복을 줄이고 관리하기 쉽게 만든다. 메서드는 선언부와...
